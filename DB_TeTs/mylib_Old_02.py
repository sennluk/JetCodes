...
def mydata(shot):
    ...
    if not icrh.ok():                     # Se il dato esiste/è ok
        ...
        icrh = V2d(r=np.zeros((50,1)), t=np.array([40.0, 80.0]), v=np.array([0.0,0.0]))
        # icrh = V2d(r=np.full((1,1), np.nan), t=np.full((1,1), np.nan), v=np.full((1,1), np.nan)) #t=np.zeros_like(hrts.t) t=np.array([40.0, 80.0])
...
#         idre = np.argmin(abs(can.r - rad))
#         can_r = can.r[idre][np.newaxis]
#         can_rt = can_v[idre,:][np.newaxis,:] # dato calcolato in Rnelll'intervallo di tempo selezionato
#     else:
#         can_t = np.full((1,1), np.nan)
#         can_v = np.full((1,1), np.nan)
#         can_r = np.full((1,1), np.nan)
#         can_rt = np.full((1,1),np.nan)
#     return can_t, can_rt   
