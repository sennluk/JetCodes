...
def def_range(d,vars):
    ...
    rhoEce = vars['rhoEce']
    #########
    interv = 0.05 # intervallo sul quale fare la media/2
    time = tlim
    psi_ts_ = psiTs.v[:,np.argmin(abs(tTs.t-time))] # valore di psi hrts all'istante scelto(_ finale per 'provvisorio')
    ...
    psi_ece_ = psiKk1[:,np.argmin(abs(tEce.t-time))] # valore di psi ece all'istante scelto(_ finale per 'provvisorio')
    ...
    if mr_ts<mr_ece:           # se il minimo della rho-hrts è più piccolo del minimo della rho-ece
        psi1 = mp_ts - 0.001   # prendo come psi1 le psi-hrts a cui sottraggo un piccolo valore
        pos = pos_mr+interv     # posizione del minimo più 5 centimetri
        idx = np.argmin(abs(tTs.r-pos))
        psi2 = psi_ece_[idx]+0.001
        
        psi1 = round(max(mp_ts - 0.001, 0),4)
        psi2 = round(psi_ece_[np.argmin(abs(tEce.r-pos))],4)
        rho1 = round(max(mr_ts - 0.001, 0),4)
        rho2 = round(rho_ece_[np.argmin(abs(tEce.r-pos))],4)
...
# Figura per seminario:    
    fig00,(ax00) = plt.subplots(nrows=1, sharex=True, num=f'{shot} - Time trend at R')
    ax00.lw = 0.5
    ax00.plot(timeTs, tempTs, color = 'darkorange', lw = linew, label='Tts')
    ax00.errorbar(timeEce, tempEce, color = 'forestgreen', lw = linew, label='Tece')
...
def rhofig(d, vars): 
    ...
    plt.ylabel('RHO')
    plt.legend()
    plt.title(f'JPN {shot} - Check plot: RHO-TORn vs Normalized Minor Radius from FLUSH(HRTS chann)')  
    if d['savefigs'] == 1: 
...
def fig_psi_mean(d, vars): 
    ...
    fig005, ax005 = plt.subplots(nrows=1, sharex=True, num=f'{shot} - smProfile -PSI- Time trend - Bands')
    # err_xm va diviso per due o no?, cosa FA L'ERROR BAR DEL PLOT PRECEDENTE?
    err_xm = np.array(err_xm)
    linew = 0.7
    ax005.lw = 0.5
...
def multiplot(d,vars): 
    ...
    btor = w.magn.bvac   # Total NBI Power
    ...
    ax2.plot(tIpla, ipla, lw = linew, color='b', label= r'$I_pla$')
    ...
    if d['savefigs'] == 1: 
        plt.savefig(f'JPN{shot}_Multiplot.pdf',dpi=300)
