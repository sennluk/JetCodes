...
V05: Aggiungo barra degli errori (divido il valore dell'errore/2 e /1000
in entrambi i plot - Metto un controllo nel caso ci siano valori troppo elevati, 
li riporto a zero
 
!!!!!!!   NON FUNZIONA LA SECONDA PARTE    !!!!!
--> saltare questa versione!
"""
import numpy as np
...
shot = 104522
tlim1 = 49  # limite inferiore selezione tempi - in secondi
tlim2 = 50 
...
ErrY = Errts.v[0,:]/2000 # Divido per due perchè la barra è doppia, /1000 per keV
...
indexTS =  (Tts.t >= tlim1) & (Tts.t <= tlim2) 
Tts.t = Tts.t[indexTS]
Tts.v = Tts.v[:,indexTS]
id_r = np.argmin(abs(Tts.r - rad))
Tts.r = Tts.r[id_r][np.newaxis]
Tts.v = Tts.v[id_r,:][np.newaxis,:]
p = Tts.v/1000
ErrY = ErrY [indexTS]

indexECE = (Tece.t >= tlim1) & (Tece.t <= tlim2) # Definisco gli indici 
Tece.t = Tece.t[indexECE]
q = Tece.v[0,indexECE]/1000



dim2 = Tts.t.size
v,t = resample(Tece.v, dim2, t=Tece.t, axis=1)
Tece.v = v
Tece.t = t
...
# plt.errorbar(p,q,yerr = ErrY,marker='o', markersize=3,ecolor='g',linestyle='none', 
