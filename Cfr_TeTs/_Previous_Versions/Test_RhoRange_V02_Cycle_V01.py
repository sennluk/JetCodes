...
N.B.: l'Ece è molot più veloce, qiuindi ci sarà sempre un punto vicino temporalmente'
"""
...
temp_tsM_rho = np.zeros([rhoTs2.shape[0],timeTs2.shape[0]])      #inizializzo matrice Tts valori temp in t1-t2
err_tsM_rho = np.zeros([rhoTs2.shape[0],timeTs2.shape[0]])  #inizializzo matrice errTs valori Err-temp in t1-t2 

# temp_eceM_rho = np.zeros([rhoEce2.shape[0],temp_ece2.shape[1]])
# rho_eceM_rho = np.zeros([rhoEce2.shape[0],temp_ece2.shape[1]])
# err_eceM_rho = np.zeros([rhoEce2.shape[0],temp_ece2.shape[1]])
...
err_ece22 = np.zeros_like(temp_ece22)

# temp_eceM_rho = np.zeros([rhoEce22.shape[0],timeTs2.shape[0]])
temp_eceM_rho = np.zeros_like(temp_ece22)
rho_eceM_rho = np.zeros([rhoEce22.shape[0],timeTs2.shape[0]])
err_eceM_rho = np.zeros([rhoEce22.shape[0],timeTs2.shape[0]])
...
for i,inst  in enumerate(timeTs2):
    ...
    r = rDiag[ind]     # Le posiszioni della diagnostica di cui sopra
    ...
    for t in range(0,(rhoTs2.shape[0])):
        mask = (rhoTs2[:,t] >= rhodown) & (rhoTs2[:,t] <= rhoup)    # Seleziono gli indici dei tempi di interesse 
        temp = temp_ts2[mask,t]                    # Valori di temperaatura nelle rho selezionate
        sig = err_ts2[mask,t]       # Errore sui singoli valori di temperatura: sigma
    ...
    for t in range(0,(rhoEce22.shape[0])):
        mask1 = (rhoEce22[:,t] >= rhodown) & (rhoEce22[:,t] <= rhoup)    # Seleziono gli indici delle psi di interesse 
        temp_eceM_rho[t]  = np.mean(temp_ece22[mask1,i], axis=0)
        rho_eceM_rho[t] = np.mean(rhoEce22[mask1,i],axis=0)
        err_eceM_rho[t] = np.mean(err_ece22[mask1,i], axis=0)
...
ax[1].legend()
ax[2].plot(timeTs2, temp_tsM_rho[20,:], label='TS averaged')
ax[3].plot(timeEce22,temp_eceM_rho[20,:])
