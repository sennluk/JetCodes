...
# Selezione intervallo temporale e posizione radiale per il TS - Lidar
idt = (Tlid.t >= tlim1-delta) & (Tlid.t <= tlim2+delta) # Seleziono gli indici dei tempi di interesse
idr = np.argmin(abs(Tlid.r - rad))    # Seleziono gli indici dei raggi di interesse (il più vicino)
TimeLID = Tlid.t[idt]
TempLID = Tlid.v[:,idt]
TempLID = TempLID[idre,:][np.newaxis]
RadLID = Tlid.r[idre][np.newaxis]
#################
################
