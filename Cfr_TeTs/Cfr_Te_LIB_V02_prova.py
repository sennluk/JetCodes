...
# si calcola dove la temperatura sale siupra Tref (con la media mobile, evitando gli spike),
# e poi quando ritoran a tale valore dopo essere passata per un massimo di Tmax, 
# calcolato du un intervallo compreso tra il ti appena calcolato e un istante finale 
# considerrato a massimo 60 secondi    
...
def tdef(d,vars):
    ...
    def find_return_point_np(data, start_index, target_value):
        ...
        for i in range(start_index + 1, len(data)):
            ...
            data_subset = data[rising_index + 1:]  # Prendi solo la parte dopo start_index
    ...
    if rising_index is not None:
        ...
        valid_indices = np.where((tmax.t > ti) & (tmax.t <= 60))[0]
        tlim = tmax.t[np.nanargmax(tmax.v[0,valid_indices])] 
        ind_tlim = np.nanargmax(tmax.v[0,valid_indices])
        TMAX = data[ind_tlim]
        data2 = data[valid_indices]
        # Trova il punto di ritorno
        return_index = find_return_point_np(data, ind_tlim, rising_value-0.2)
        # return_index = find_return_point_np(data, rising_index+1, rising_value-0.2)
    ...
    # valid_indices = np.where((tmax.t > ti) & (tmax.t <= tf))[0]
    # tlim = tmax.t[np.nanargmax(tmax.v[0,valid_indices])] 
    # ind_tlim = np.nanargmax(tmax.v[0,valid_indices])
    # TMAX = data[ind_tlim]
