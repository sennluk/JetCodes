...
def dictvar(d):
    ...
    vars = {
        ...
        'temp_tsM' : None, 
        'err_tsM': None,
        'rhodown' : None,  # As calculated by the automatic procedure - TS interpolated in t1<t<t2
        'rhoup' : None,    # As calculated by the automatic procedure - TS interpolated in t1<t<t2
        'xm': None, 
        'err_xm' : None, 
        'temp_eceM' : None, 
        'err_eceM' : None, 
...
# Vedo quale ha il minimo più alto e lo prendo come psi1 (tolgo qualcosa in più,
...
def psifig(d,vars):
    ...
    # Sequenza di esempio (modifica qui se necessario)
    sequenza = np.array([False, False, True, True, False, True, True, False])
...
def rhofig2(d, vars): 
    ...
    plt.ylabel('RHO')
    plt.legend()
    plt.title(f'JPN {shot} - Check plot per il calcolo di RHO: RHO-TORn HRTS vs Normalized Minor Radius from FLUSH(HRTS chann)')  
    if d['savefigs'] == 1: 
       plt.savefig(d['mypath']+f'{shot}_RHO-torN vs NomrMinRad.pdf',dpi=300)
...
def fig_cfr_psi(d, vars): #shot, w, tlim1, tlim2, delta, psi1, psi2, xm, err_xm, temp_eceM, err_eceM):  
    ...
    ax06.set_xlabel('Te HRTS (keV)')
    ax06.set_ylabel('Te Ece-Michelson (keV)')
    #fig06.tight_layout()
...
def mean_calc_rho(d, vars): 
    ...
    temp_tsM_rho = []   # temp media in intervallo rho
    ...
    ### Medie dell'ECE sull'intervallo di rho scelto      
    ...
    # Calcolo delle medie nell'intervallo di rho tra rho1 e rho2 nell'intervallo di tempo scelto
...
def cfr_harm(d, vars): #shot, w, tlim1, tlim2, delta, psi1, psi2, xm, err_xm, temp_eceM, err_eceM):  
    ...
    ax.legend()
    ax.set_title(f'JPN {shot} 2nd vs 3rd harmonic Trad')
    fig.tight_layout()
    ...
    plt.figure('cfr')
    plt.scatter(tempEce[25,:],tRad_2nd/1000)

###############################################################   
# show the comparisno with respect to flux coordinate choice in one plot
    
# def plot_comp(d,vars)