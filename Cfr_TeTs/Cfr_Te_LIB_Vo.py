...
def def_range_av(d,vars):
    ...
    print('Dimensioni (nell intervallo temporale ti-tf) di rhoTs = ',rhoTs2.shape)
    print('Dimensioni (nell intervallo temporale ti-tf) di rhoEce = ',rhoEce2.shape)
    ...
    for i,inst  in enumerate(timeTs2):
        ...
        #### Calcolo medie istante per istante
    ...
    print("Ciclo terminato!")  # Questo è fuori dal ciclo
    pippo = temp_tsM_rho
    pluto = temp_eceM_rho
    ...
    vars['temp_tsM_rho'] = pippo
    vars['temp_eceM_rho'] = pluto
    
    vars['timeTs2'] = timeTs2
    vars['timeEce22'] = timeEce22
    vars['ranges'] = ranges       # RHH values for every HRTS instant
    # vars['temp_tsM_rho'] = temp_tsM_rho # Te HRTS averages in the selected RHO range
    vars['err_tsM_rho'] = err_tsM_rho  # Te-errors HRTS averages in the selected RHO range
    # vars['temp_eceM_rho'] = temp_eceM_rho # Te ECE averages in the selected RHO range
    vars['err_eceM_rho'] = err_eceM_rho  # Te-errors ECE averages in the selected RHO range   
    
    print("vars scritte?")  # Questo è fuori dal ciclo
