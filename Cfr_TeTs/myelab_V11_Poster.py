...
def magax(d,vars):
    ...
    plt.ylabel('z(m)')
    plt.legend()
    plt.title(f'Position of the magnetic axis on the poloidal plane - from EFIT')
...
##################################################
# V11: provo a selezionare solo valori comuni, 
# e assgno un intervallo direttamente in PSI
# Definisco i range di valori di PSI e RHO su cui fare le analisi:
# Prendo gli andamenti di psi per HRTS e ECE.
# Vedo quale han il minimoo più alto e lo prendo come psi1 (tolgo qualcosa in più,
# il valore di fix, che considero come tolleranza di accettazione per il confronto tra le due)
# Poi prendo come Psi 2 il valore di psi 1 + intPsi
...
def def_range(d,vars):
    ...
    minP_ece = min(psiEce_s)
    minP_ts = min(psiTs_s)
    pip = max(minP_ece, minP_ts)
    if minP_ts<minP_ece:
        psi1_ = psiTs_s[np.where(min(-(psiTs_s-minP_ece)))]
        pos_psi1 = psiTs.r[np.where(psiTs_s == psi1_)]
        psi2_= psiEce_s[np.where(min(-(psiEce_s-psiTs_s[0])))]
        pos_psi2 = tEce.r[np.where(psiEce_s == psi2_)]
    else:
        psi1_ = psiEce_s[np.where(min(-(psiEce_s-minP_ts)))]
        psi2_ = psiTs_s[0]
    
    
    # ##########old vers:
    # # intPsi = 0.1 # intervallo sul quale fare la media/2 espressi in psi
    # fix = 0.01 # termine aggiuntivo per sicurezza valori psi e rho
    # time = tlim
    ...
    #     ind = psi_ts[min(-(psi_ts_[:]-mp_ece]))
    #     # psi1_ = mp_ece - fix   # prendo come psi1 imaggiore tra i due minimi a cui sottraggo un piccolo valore
    #     # pos = pos_mpe     # posizione del minimo più interv (10 centimetri?)
    #     # psi2_ = psi1_ + intPsi
    ...
    # else:
    #     psi1_ = mp_ts - fix   # prendo come psi1 le psi-ece a cui sottraggo un piccolo valore
    #     pos = pos_mpe      # posizione del minimo 
    #     psi2_ = psi_ts_[1]    # Prendo come Psi2 il promo valore di Psi_ts all'istante scelto
    ...
    print('Computed psi1 = ', psi1_)#, 'in pos =', pos_mp)
    print('Computed psi2 = ', psi2_)#, 'Approx range = ', 2*interv*100, 'cm')
...
def psifig(d,vars):
    ...
    ax01.set_ylabel('PSI')
    ax01.legend()
    ax001.set_title(f'PSI(R) for HRTS and ECE-KK1 at t={tlim} sec')
    fig01.tight_layout()
    ...
    ax02.legend()
    ax02.set_title(f'Te(PSI) profile at t={tlim} sec')
    fig02.tight_layout()
...
def rhofig(d, vars): 
    ...
    ax03.legend()
    ax003.set_title(f'RHO(R) for HRTS and ECE-KK1 at t=10 sec')
    fig03.tight_layout()
...
def fig_psi_mean(d, vars): 
    ...
    ax04.set_title(f'Mean (Ece and Hrts) and w-Mean(Hrts) for {psi1}<PSI<{psi2}')
    ...
    ax05.set_title(f'Ece-Mean and HRTS w-Mean for {psi1}<PSI<{psi2} vs time ')
    ...
    linew = 0.7
    ax005.lw = 0.5
    ax005.plot(time_ts-40,xm_sm,lw=linew,color='darkolivegreen',label='HRTS w-mean')
    ax005.fill_between(time_ts-40,xm_sm+err_xm_sm,xm_sm-err_xm_sm, color='g',alpha=0.3) # xm is the weighted mean for HRTS
    ax005.plot(time_ece-40,temp_eceM_sm,lw = linew, color='orange', label='ECE mean' )             
    ax005.fill_between(time_ece-40,temp_eceM_sm+err_eceM_sm,temp_eceM_sm-err_eceM_sm,color = 'darkorange',alpha=0.3)
    ax005.axvline(x = tlim1-40,c='r',ls='--',lw=.5)
    ax005.axvline(x = tlim2-40,c='r',ls='--',lw=.5)
    ax005.legend(fontsize=8)
    ax005.set_title(f'Smooted Ece and HRTS w-Mean for {psi1}<PSI<{psi2} vs time ')
...
def fig_cfr_psi(d, vars): #shot, w, tlim1, tlim2, delta, psi1, psi2, xm, err_xm, temp_eceM, err_eceM):  
    ...
    p = xm12            # Valori TeHRTS media-pesata calcolata sull'intervallo di psi specificato
    q = temp_eceM_R   # Valori TeECE media calcolata sull'intervallo di psi specificato
    ...
    ax06.legend()
    ax06.set_title(f'Te HRTS vs Te Ece-Michelson.{psi1}<PSI<{psi2} for {tlim1:.2f}<t<{tlim2:.2f} (s)')
...
def fig_cfr_rho(d, vars): #shot, w, tlim1, tlim2, delta, psi1, psi2, xm, err_xm, temp_eceM, err_eceM):  
    ...
    p = temp_tsM_rho             # Valori TeHRTS media-pesata calcolata sull'intervallo di psi specificato
    q = temp_eceM_rho_R   # Valori TeECE media calcolata sull'intervallo di psi specificato
    # Plot della Te TS vs Te ECE + retta x=y
    ...
    ax07.legend()
    ax07.set_title(f'Te HRTS vs Te Ece-Michelson. \n {rho1}<RHO<{rho2} for 7<t<9 (s)') #:.2f per avere 2 cifre decimali
    # ax07.set_title(f'Te HRTS vs Te Ece-Michelson. \n {rho1}<RHO<{rho2} for {tlim1}<t<{tlim2} (s)')
    ax07.set_xlabel('Te HRTS (keV)')
    ax07.set_ylabel('Te Ece-Michelson (keV)')
    # fig07.tight_layout()
...
def multiplot(d,vars): 
    ...
    fig, (ax1, ax2, ax3, ax4, ax5, ax6,  ax8) = plt.subplots(nrows=7, sharex=True, num=f'Trends over time')
    plt.subplots_adjust(hspace=0)
    
    ax1.plot(tBtor-40,btor, lw = linew, label='$B_{tor}$')
...
###############################################################   
# Calculates the radiation temperature from the calibrated ece spectrum, 
# for the 2nd and the 3rd harmonic X-mode ECE for all the data available, 
# then perform the direct comparison between the two.
# Frequency of the second harmonic is obtained from the fmax given by the ecm1/ppf channel

def cfr_harm(d, vars): #shot, w, tlim1, tlim2, delta, psi1, psi2, xm, err_xm, temp_eceM, err_eceM):  
    shot = d['shot']
    tlim1 = d['tlim1']
    tlim2 = d['tlim2']
    tEce = vars['tEce'] 
    ...
    # timeEce = tEce.t
    # idt = (timeEce >= tlim1) & (timeEce <= tlim2) # Seleziono gli indici dei tempi di interesse
    # temp_2nd = tEce.v[:,idt]/1000  # temperatura in keV  seconda armonica dal canale prfl
    
    w = ppfs(shot) 
    c = 299792458         # vel luce in m/s 
    ec = 1.602176634e-19   # unità di carica in Coulomb
    
    tempEce = tEce.v
    
    spec_ece = w.ecm1.cspc   # calibrated ECE spectrum (intensity in pW*Hz^-1*SteRad^-1*m^-2)
    fmax = w.ecm1.fmax   # posizione del massimo in frequenza: freq ciclotronica in Hz
    fcom = w.ecm1.fcom   # center of mass -posizione del massimo che permette di evitare gli spike qualora ce ne fossero
    
    tRad = np.ones(spec_ece.v.shape)
    tRad_2nd = np.ones(len(spec_ece.t))
    tRad_3rd = np.ones(len(spec_ece.t))
    for i in range(0,len(spec_ece.t)):
        for j in range (0,(len(spec_ece.r))):
            tRad[j,i] = (spec_ece.v[j,i]*c**2)/(1e12*ec*spec_ece.r[j]**2)
            
        ind2 = np.argmin(abs(spec_ece.r-2*fcom.v[0,i]))
        ind3 = np.argmin(abs(spec_ece.r-2.9*fcom.v[0,i]))
        tRad_2nd[i] = tRad[ind2,i]/1000
        tRad_3rd[i] = tRad[ind3,i]/1000  # possible factor 1.12*
    
    dims = len(tRad_2nd)    
    left = min(np.nanmin(tRad_2nd), np.nanmin(tRad_3rd))
    right  = max(np.nanmax(tRad_2nd), np.nanmax(tRad_3rd))
    def retta(x):
        return x
    x = np.linspace(left-0.5,right+0.5,dims)   # Range in keV di dove tracciare la retta
    y = retta(x)  
    
    fig, ax1 = plt.subplots(nrows=1, sharex=True, num='T2nd & T3rd vs time')
    ax1.plot(spec_ece.t, tRad_2nd, label= 'T2nd')
    ax1.plot(spec_ece.t, tRad_3rd, label= 'T3rd')
    ax1.set_xlabel('time(sec)')
    ax1.set_label('T (keV)')
    ax1.set_title('Trad of the 2nd and 3rd Xmode harmonic ECE vs time')
    ax1.legend()
    
    fig, ax = plt.subplots(nrows=1, sharex=True, num = '2vs3')
    ax.scatter(tRad_2nd, tRad_3rd, marker='1', lw = 0.7, facecolors='olive', edgecolors='darkorange', label='2vs3') #s= 5, 
    ax.plot(x,y, lw= 0.5, color='r')
    ax.set_xlabel('2nd harmonic Trad (keV)')
    ax.set_ylabel('3rd harmonic Trad (keV)')
    ax.legend()
    ax.set_title(f'2nd vs 3rd harmonic Trad')
    fig.tight_layout()
    
    if d['savefigs'] == 1: 
        plt.savefig(d['mypath']+f'{shot}_2vs3.pdf',dpi=300)
